...
func main() {
	...
	flag.StringVar(&kafkaBrokerURL, "kafkaBroker", "192.168.99.100:19092", "URL of kafka broker")
	...
	go func() {
		log.Info().Msgf("starting server at %s\n", listenAddrAPI)
...
func postDataToKafka(ctx *gin.Context) {
	...
	ctx.Bind(form)
	formInBytes, err := json.MarshalIndent(form, " ", " ")
	if err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, map[string]interface{}{
			"error": map[string]interface{}{
				"message": fmt.Sprintf("error while marshalling json: %s\n", err.Error()),
			},
		})
	...
	if err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, map[string]interface{}{
			"error": map[string]interface{}{
				"message": fmt.Sprintf("error while push message into kafka: %s\n", err.Error()),
			},
		})
